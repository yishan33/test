...
{
    [self sd_setImageWithURL:url placeholderImage:nil options:0 progress:nil completed:nil prune:YES withPercentage:percentage];
}
...
- (void)sd_setImageWithURL:(NSURL *)url placeholderImage:(UIImage *)placeholder options:(SDWebImageOptions)options progress:(SDWebImageDownloaderProgressBlock)progressBlock completed:(SDWebImageCompletionBlock)completedBlock prune:(BOOL)doPrune withPercentage:(CGFloat)percentage{
    [self sd_cancelCurrentImageLoad];
    objc_setAssociatedObject(self, &imageURLKey, url, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        ...
        id <SDWebImageOperation> operation = [SDWebImageManager.sharedManager downloadImageWithURL:url options:options progress:progressBlock completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished, NSURL *imageURL) {
            ...
            dispatch_main_sync_safe(^{
                ...
                if (image) {
                    ...
                    __block UIImage *tinyImage = [UIImage imageWithCGImage:CGImageCreateWithImageInRect([image CGImage], rect)];
                    wself.image = tinyImage;
                    [wself setNeedsLayout];
                } else {
                    if ((options & SDWebImageDelayPlaceholder)) {
                        wself.image = placeholder;
